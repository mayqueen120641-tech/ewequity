...
- **ECOS 차단**: 한국은행이 GAS 서버 IP를 종종 차단. 이때 에러가 바로 나는 게 아니라 응답이
  올 때까지 ~50초를 매달린다. 실패 시 환율은 Yahoo(`KRW=X`), 기준금리는 스크립트 속성
  수동값(`BASE_RATE_KR_MANUAL`)으로 폴백.
- **`UrlFetchApp.fetchAll()`의 함정**: `muteHttpExceptions`는 HTTP 4xx/5xx만 막아준다.
  ...
  예외를 던져 배치 전체 응답을 잃는다**. 그래서 `fetchJobsSafe_()`가 배치 실패 시 `flaky`로
  표시된 요청(ECOS)만 빼고 나머지를 한 번 더 **병렬로** 재요청한다. 순차 재시도로 만들면
  폴백 경로가 오히려 더 느려지니 주의.
- **ECOS 회로차단기**: ECOS가 막힌 게 확인되면 `ecos_down` 캐시 키로 30분간 아예 시도하지
  않는다(`ECOS_DOWN_KEY_`/`ECOS_DOWN_SEC_`). 덕분에 `getRates`가 2~3초로 유지됨. 다시
  시도하게 하려면 `clearAllCaches()` 실행.
...
1. ~~**지표 로딩 속도 개선**~~ (2026-07-28 완료) — 지표 8종을 `fetchAll()` 병렬 요청으로
   전환 + ECOS 회로차단기 추가. 측정값: 평상시 **2~3초**. 다만 ECOS가 막히는 순간을 맞은
   요청 1건은 여전히 ~50초를 문다(그 후 30분은 빠름). 이걸 없애려면 시간 기반 트리거로
   ECOS를 백그라운드에서 미리 받아 캐시에 넣고, `doGet`은 ECOS를 아예 건드리지 않는 구조로
   가야 함 — 다음 후보.
